//
//  GrowthDataCalculator.swift
//  babyMoa
//
//  Created by Baba on 11/13/25.
//

import Foundation

struct GrowthDataCalculator {

    /// 기록 날짜와 아기 생년월일을 기반으로 월령 레이블을 계산합니다.
    /// - Parameters:
    ///   - recordDate: 키/몸무게 기록이 측정된 날짜.
    ///   - birthDate: 아기의 생년월일.
    /// - Returns: "X개월" 형식의 문자열 또는 계산할 수 없는 경우 빈 문자열.
    static func calculateMonthLabel(recordDate: Date, birthDate: Date) -> String {
        let calendar = Calendar.current
        // 아기 생년월일과 기록 날짜 사이의 월(month) 구성 요소를 계산합니다.
        // .day 구성 요소를 포함하여 월 계산 시 일(day) 단위까지 고려하여 정확도를 높입니다.
        let components = calendar.dateComponents([.month, .day], from: birthDate, to: recordDate)
        
        if let months = components.month {
            // 일(day) 구성 요소가 15일 이상이면 반올림하여 다음 달로 간주합니다.
            // 이 로직은 필요에 따라 조정될 수 있습니다.
            if let days = components.day, days >= 15 {
                return "\(months + 1)개월"
            }
            return "\(months)개월"
        }
        return "" // 계산할 수 없는 경우 빈 문자열 반환
    }

    /// 현재 값과 이전 값을 기반으로 차이 텍스트를 계산합니다.
    /// - Parameters:
    ///   - currentValue: 현재 키/몸무게 값.
    ///   - previousValue: 이전 키/몸무게 값 (선택 사항).
    /// - Returns: "+X.X" 또는 "-X.X" 형식의 문자열 또는 차이가 없거나 이전 값이 없는 경우 nil.
    static func calculateDiffText(currentValue: Double, previousValue: Double?) -> String? {
        guard let previousValue = previousValue else { return nil }
        let diff = currentValue - previousValue
        
        if diff > 0 {
            return String(format: "+%.1f", diff) // 예시: "+0.1"
        } else if diff < 0 {
            return String(format: "%.1f", diff) // 예시: "-0.1"
        } else {
            return nil // 차이 없음
        }
    }
}
