...
struct WeightAddView: View {
    ...
    @StateObject private var viewModel: WeightAddViewModel
    @FocusState private var isFocused: Bool           // 메모 TextEditor 포커스
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ScrollViewReader { proxy in
                    ScrollView {
                        VStack(alignment: .leading, spacing: 24) {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                Button {
                                    ...
                                    HStack {
                                        Spacer()
                                        Text("\(String(format: "%.1f", viewModel.weightValue)) kg")
                                            .font(.system(size: 16, weight: .medium))
                                            .foregroundColor(Color("Font"))
                                        Spacer()
                                    }
                            ...
                            HorizontalDialPicker(
                                ...
                                range: viewModel.minWeight...viewModel.maxWeight,
                                step: 0.1
                            )
                        ...
                        .padding(.bottom, 44)                // ✅ 하단 여유(키보드 위로)
                    }
                    .scrollIndicators(.hidden)
                    .scrollDismissesKeyboard(.interactively)

                    // ✅ 메모 포커스 시 저장 버튼까지 자동 스크롤
            ...
            .padding(.bottom, 44)
            // ✅ 아무 곳이나 탭하면 키보드 내리기 (버튼 동작 방해 없음)
            .simultaneousGesture(
...
#Preview {
    WeightAddView(coordinator: BabyMoaCoordinator(), babyId: 1)
}
